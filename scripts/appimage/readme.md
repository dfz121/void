...
stable-linux.yml in the separate `vs-aware-builder/` repo,
which runs a GitHub Action that builds the AppImage you see on our website.


# VS Aware AppImage Creation Script

This script automates the process of creating an AppImage for the VS Aware Editor using Docker. It works on macOS and Linux platforms.
## Requirements
...
5.  **Result:**
    ...
    After the script completes, it will generate an AppImage named `VSAware-x86_64.AppImage` (or similar, depending on your architecture) in the current directory.
...
*   **AppImage Creation:**
    *   Creates the `VSAwareApp.AppDir` structure.
    *   Copies binaries, resources, and the `.desktop` entry.
    *   Copies `vs-aware.desktop` and `vs-aware.png`.
